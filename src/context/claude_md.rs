...
mod tests {
    ...
    fn repository_vm_capsule_loads_once_after_the_root_instructions() {
        // The real tree: root CLAUDE.md with its AGENTS.md alias, and the finch-vm capsule as a real
        // AGENTS.md beside a CLAUDE.md that only imports it. An agent working in finch-vm must get
        // the root instructions and then the capsule, each exactly once.
        let repo = Path::new(env!("CARGO_MANIFEST_DIR"));
        let home = TempDir::new().unwrap();
        let sources = collect_instructions(&repo.join("crates/finch-vm/src"), Some(home.path()));
        let loaded: Vec<&Path> = sources.loaded().collect();
        let root = repo.join("CLAUDE.md");
        let capsule = repo.join("crates/finch-vm/AGENTS.md");
        ...
        assert!(
            ...
            "finch-vm must load the root instructions and then the vm capsule; loaded={loaded:?}"
        );
        ...
        let text = sources
            .text()
            .expect("instructions must load from finch-vm");
        assert_eq!(
