...
impl TuiRenderer {
    ...
    /// Construct a renderer for production-boundary tests without claiming or
    /// mutating the process terminal. The event loop still exercises the real
    /// renderer state and output manager; only terminal ownership is omitted.
    #[cfg(test)]
