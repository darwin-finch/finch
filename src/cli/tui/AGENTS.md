...
area, scrollback, accordion/transcript projection, activity rows, and the graph overlay view.
...
| [`GraphView`](graph.rs) | nodes, edges, camera | Finch `Poset`, in `graph_view_from_poset` beside `set_poset` |
...
Splitting the poset widget out of the framework (rather than generalising it) remains open. The
widget now draws `GraphView`; Finch's `Poset` is kept only at the injection boundary because
`EventLoop` still hands the shared mutex in and this capsule must not rewrite `repl_event`.
...
- **`crate::poset::Poset` in `mod.rs` only** — `TuiRenderer::set_poset` and
  `graph_view_from_poset`. The event loop shares `Arc<Mutex<Poset>>` with the executor; converting
  at `set_poset` is the injection. The overlay draws `GraphView`. Do not add new `crate::poset`
  names outside that boundary (`test_tui_production_keeps_finch_poset_at_the_injection_boundary`).
...
`crate::tools` or `crate::runtime` name.
