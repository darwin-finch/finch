...
- `F6`/`Shift+F6` moves semantic focus, Left/Right collapses or expands,
  Enter/Space toggles, and Escape or any unrelated composer key returns focus
  to the draft. Left-click toggles directly without stealing keyboard focus
  from the composer when terminal mouse reporting is available.
