...
`cli::messages` gives one interactive Finch session typed, mutable conversation entries. A
...
1. The [query processor](../repl_event/query_processor.rs) and [event loop](../repl_event/event_loop.rs)
   retain an `Arc<WorkUnit>` while provider and tool events arrive. The [output manager](../output_manager.rs)
...
2. The [TUI adapter](../tui/view_model.rs) reads a `MessageRef` at blit time. It asks for the
...
[`finch-ui-model`](../../../crates/finch-ui-model/AGENTS.md); this module currently re-exports
some of those types for application compatibility. Read [AGENTS.md](AGENTS.md) for lifecycle and
dependency rules, then [`mod.rs`](mod.rs) for the callable facade. Rustdoc supplies signatures
