...
pub(super) async fn watch_named_brain(
    ...
    Ok(ws
        .on_upgrade(move |mut socket| async move {
            ...
            if let Ok(encoded) = crate::brain::encode_brain_remote_envelope(&initial) {
                if socket
                ...
                tokio::select! {
                    incoming = socket.recv() => match incoming {
                        ...
                        Some(Ok(WsMessage::Binary(bytes))) => {
                            match crate::brain::decode_brain_remote_envelope(&bytes) {
                                Ok(BrainRemoteEnvelope::Command(command)) => {
                                    if matches!(
                                        &command.kind,
                                        crate::brain::BrainRemoteCommandKind::Detach
                                    ) {
                    ...
                    reply = reply_rx.recv(), if replies_open => {
                        ...
                        let Ok(encoded) = crate::brain::encode_brain_remote_envelope(&envelope) else {
                            break;
                        };
                        #[cfg(test)]
