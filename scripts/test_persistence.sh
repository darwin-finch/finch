...
if [ -f ~/.finch/models/threshold_router.json ]; then
    echo "✓ Router model saved"
    echo "Router stats:"
    jq '.total_queries' ~/.finch/models/threshold_router.json
else
    echo "✗ Router model NOT saved"
fi

if [ -f ~/.finch/models/threshold_validator.json ]; then
...
if [ -f ~/.finch/metrics/$(date +%Y-%m-%d).jsonl ]; then
    echo "✓ Metrics file exists"
    echo "Number of logged queries:"
    wc -l ~/.finch/metrics/$(date +%Y-%m-%d).jsonl

    echo ""
    echo "Sample metric (first entry):"
    head -1 ~/.finch/metrics/$(date +%Y-%m-%d).jsonl | jq '.' || true
else
