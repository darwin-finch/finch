...
- [ ] Generalize provider authentication beyond static API tokens. Add secure-store-backed OAuth,
  ...
  account flows), alongside API keys and local/no-auth runtimes. Credentials must be opaque references with
...
- [ ] Stop tests and smoke checks from polluting the user's real Brain store. Every unit,
  integration, conformance, bootstrap, handoff, cancellation, and daemon-upgrade fixture must bind
  an explicit temporary Finch home/Brain root; fail a test that would resolve to the invoking
  user's `~/.finch/brains`. Clean temporary stores after success and failure, and add a regression
  proving the real named-Brain list is byte-for-byte unchanged by the test suite.
- [ ] Add safe Brain lifecycle cleanup and expose it through the typed VM. Automatically remove an
  ephemeral home Brain only when its final attachment leaves and it has no durable conversation,
  task, approval, schedule, retained program, runtime revision, grant, or audit state. Provide typed
  `brain-list`, `brain-inspect`, `brain-archive`, and separately authorized `brain-delete`
  operations with filter/predicate composition, dry-run results, stable Brain IDs, exact expected
  revisions, and capability/audit enforcement. Bulk cleanup generated by an LLM must remain a
