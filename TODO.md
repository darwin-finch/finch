...
- [ ] Replace the compatibility output adapter with portable output-handle effects: default response
  ...
  wire effects now cross the client's ordered REPL event bus before any WorkUnit mutation; still replace
  the remaining direct compatibility projections with durable application-journal/replay support.
