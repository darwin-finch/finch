...
use crate::{
    ...
    ProviderCredential, ProviderPorts, ResolvedCredential, ResolvedSecret,
};
    ...
    S: OAuthCredentialStore + 'static,
{
    pub fn new(dialect: Arc<D>, store: Arc<S>) -> Result<Self> {
        Self::with_ports(dialect, store, ProviderPorts::production()?)
    }
    ...
    pub async fn finish_device_authorization_commit(
        &self,
        reference: &str,
        pending: &DeviceAuthorization,
        cancel: CancellationToken,
    ...
    async fn post_form_bytes_cancellable(
        ...
        self.ports.http.post(request, self.timeout, cancel).await
    }
    ...
    async fn post_revoke(
        ...
        let (status, _body) = self
            .ports
            .http
            .post(request, self.timeout, cancel)
            .await
