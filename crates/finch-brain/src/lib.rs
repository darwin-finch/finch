...
// `crate::Item` and never name a child module.
pub use attachment::{
...
pub use ipc_codec::{
    ...
    decode_messages, decode_run, decode_runner_handoff, decode_runner_lease, decode_schedule,
    decode_snapshot, encode_approval_audience, encode_attachment, encode_brain_remote_envelope,
    encode_brain_submission, encode_brain_submission_outcome, encode_continuation_messages,
    encode_environment, encode_event, encode_invocation_metadata, encode_messages, encode_run,
    encode_runner_handoff, encode_runner_lease, encode_schedule, encode_snapshot,
    run_status_from_capnp, run_status_to_capnp, BrainRemoteCommand, BrainRemoteCommandKind,
...
pub mod test_support {
    ...
    pub use super::ipc_codec::{
        brain_remote_command_fingerprint, decode_brain_remote_envelope, decode_brain_submission,
        encode_brain_remote_envelope, encode_brain_submission, BrainRemoteCommand,
        BrainRemoteCommandKind, BrainRemoteEnvelope, BrainRemoteMutation, BrainRemoteReply,
    };
...
pub struct IsolatedTestProof {
    // Clone is implemented by hand below, not derived: a derived impl is
    // exempt from dead-code analysis, which would then flag every field
    // this struct carries purely for later test-only verification
    // (`root_identity`, `socket_root`, `socket_root_identity`,
    // `supervisor_pid`) as unread in a non-test build.
    pub home: std::path::PathBuf,
    pub root: std::path::PathBuf,
    pub home_identity: (u64, u64),
    pub root_identity: (u64, u64),
    pub brain_addr: String,
    pub daemon_addr: String,
    pub ipc_socket: std::path::PathBuf,
    pub socket_root: std::path::PathBuf,
    pub socket_root_identity: (u64, u64),
    pub ipc_listener_identity: (u64, u64),
    pub supervisor_pid: u32,
    pub password_digest: String,
}

impl Clone for IsolatedTestProof {
    fn clone(&self) -> Self {
        Self {
            ...
            brain_addr: self.brain_addr.clone(),
            daemon_addr: self.daemon_addr.clone(),
            ipc_socket: self.ipc_socket.clone(),
            socket_root: self.socket_root.clone(),
            socket_root_identity: self.socket_root_identity,
            ipc_listener_identity: self.ipc_listener_identity,
...
fn isolated_test_proof_with_encoded_uncached() -> anyhow::Result<(IsolatedTestProof, Vec<u8>)> {
    #[cfg(test)]
    PROOF_VALIDATION_CALLS.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
    use anyhow::Context as _;
...
#[cfg(all(any(test, feature = "test-support"), unix))]
pub fn validate_isolated_test_socket(
...
mod isolation_tests {
    ...
    /// #858. A single `finch daemon` process under the supervised proof calls
    /// into `isolated_test_proof_if_present()` from four separate startup
    /// sites (`run_daemon`, `AgentServer::new`, `AgentServer::serve`,
    /// `prepare_ipc_listener`). Before caching, each site independently
    /// re-opened and re-hashed the full supervisor executable — a
    /// consistent ~42s delay between local-generator construction and the
    /// IPC socket bind on CI runners, isolated to exactly this redundant
    /// work (see the diagnosis on issue #858). The fix is a process-lifetime
    ...
    fn test_proof_authentication_runs_per_call_while_the_image_digest_is_memoized() {
        ...
        isolated_test_proof().expect("second proof validation must succeed");
        isolated_test_proof_if_present()
            .expect("third proof validation must succeed")
    ...
    fn isolated_proof_rejects_self_issued_environment_authority() {
        ...
        if let Ok(mode) = std::env::var(MODE_ENV) {
            ...
            signal_hostile_mode_ready();
            let start_attacker_responder =
                |key: [u8; 32],
                 marker: &std::path::Path|
                 -> (std::os::unix::net::UnixDatagram, std::process::Child) {
    ...
    fn isolated_peer_authentication_rejects_connected_outside_process() {
        ...
        let mut outsider = std::process::Command::new(std::env::current_exe().unwrap())
            .args([
                "--exact",
                "isolation_tests::isolated_peer_authentication_rejects_connected_outside_process",
                "--nocapture",
            ])
...
mod facade_scan_tests {
    ...
    fn brain_callers_use_facade_not_child_modules() {
        ...
        let root = manifest
            ...
            .expect("finch-brain must live under the workspace crates directory");
        let brain = manifest.join("src");
        ...
        assert!(
            hits.is_empty(),
            "callers outside finch-brain must use the flat Brain facade, not child modules; found: {hits:?}"
        );
    }
        ...
        for entry in entries.flatten() {
            ...
            for (index, line) in source.lines().enumerate() {
                ...
                for child in children {
                    for prefix in ["crate::", "finch::brain::"] {
