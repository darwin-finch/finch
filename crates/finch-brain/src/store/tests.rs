...
fn initialization_contract_is_persisted_and_inert_across_restart() {
    ...
    assert_eq!(before.capability_budget, finch_vm::EffectSet::pure());
...
async fn reviewed_initialization_module_is_typed_and_pure() {
    ...
    let outcome = runtime
        ...
        .await
        .unwrap();
    assert_eq!(outcome.status, finch_runtime::ExecutionStatus::Completed);
...
fn task_list_projection_survives_daemon_restart() {
    use crate::tasks::{BrainTask, BrainTaskPriority, BrainTaskStatus};
...
fn committed_memories_survive_daemon_restart() {
    use crate::CommittedMemoryRecord;
...
fn archive_removes_a_brain_but_preserves_its_log() {
    ...
    retained_runtime
        .grant_typed_capability(finch_vm::CapabilityRequirement {
            capability: finch_vm::CapabilityKind::AgentSpawn,
...
async fn named_brain_commits_a_validated_frontend_runner_checkpoint() {
    ...
    let store = BrainStore::with_root("box.local", Some(temp.path().into()));
    store.snapshot("brain").unwrap();
    let runner = finch_runtime::ProgramRuntime::new();
    let outcome = runner
        .submit_typed_only(finch_runtime::ProgramSubmission {
            language: finch_programs::ProgramLanguage::Lisp,
...
async fn named_brain_restores_scoped_authority_from_its_separate_policy_record() {
    ...
    let grant_id = runtime
        .issue_typed_capability(
            finch_vm::CapabilityRequirement {
                capability: finch_vm::CapabilityKind::AgentSpawn,
                selector: finch_vm::ResourceSelector::None,
            },
            finch_vm::GrantScope::Session { session_id },
            "test-user",
            None,
        )
        .unwrap();
    ...
    assert!(matches!(
        ...
        finch_vm::GrantScope::Session { session_id: restored_id } if restored_id == session_id
    ));
...
fn named_brain_persists_policy_changes_and_denials_without_a_vm_commit() {
    ...
    denied.insert(finch_vm::CapabilityKind::AgentSpawn);
    assert_eq!(
        runtime
            .apply_capability_policy(
                finch_vm::CapabilityPolicy {
    ...
    assert!(restored
        ...
        .is_some());
    assert!(restored
        .issue_typed_capability(requirement, finch_vm::GrantScope::Global, "test-user", None,)
        .unwrap_err()
        .to_string()
...
async fn named_brain_persists_denial_without_a_vm_commit() {
    ...
    let denied = runtime
        .resolve_typed_approval(
            ...
            finch_vm::ApprovalChoice::Deny,
            "test-user",
        )
        .await
        .unwrap();
    assert_eq!(denied.status, finch_runtime::ExecutionStatus::Failed);
    ...
    assert!(matches!(
        ...
        Some(finch_vm::AuthorizationDecision::Denied { .. })
    ));
}
...
async fn named_brain_persists_host_authorization_even_when_the_run_rolls_back() {
    ...
    let failed = runtime
        ...
        .await
        .unwrap();
    assert_eq!(failed.status, finch_runtime::ExecutionStatus::Failed);
    ...
    assert!(matches!(
        ...
        Some(finch_vm::AuthorizationDecision::Allowed { grant_id: used }) if *used == grant_id
    ));
}
...
async fn out_of_order_checkpoint_events_never_regress_a_brain_runtime() {
    ...
    let store = BrainStore::with_root("box.local", Some(temp.path().into()));
    let runtime = store.program_runtime("brain").unwrap();
    let submit = |source: &str, revision| finch_runtime::ProgramSubmission {
        language: finch_programs::ProgramLanguage::Forth,
        source_id: None,
        source: source.into(),
...
async fn legacy_restart_revision_reset_keeps_the_latest_request_state() {
    ...
    let checkpoint = runtime
        ...
        .unwrap();
    let legacy_restarted = finch_runtime::ProgramRuntime::from_checkpoint(checkpoint).unwrap();
...
fn audit_effect(sequence: u64, text: &str) -> finch_vm::VmSideEffect {
    finch_vm::VmSideEffect {
        protocol_version: 1,
        sequence,
        requirement: finch_vm::CapabilityRequirement {
            capability: finch_vm::CapabilityKind::SessionEmit,
            selector: finch_vm::ResourceSelector::None,
        },
        output: Vec::new(),
        event: finch_vm::HostSideEffect::Emit { text: text.into() },
        origin: finch_vm::SourceOrigin::generated("effect-audit-store-test"),
    }
}
...
fn effect_audit_permit_precedes_host_outcome_and_survives_turn_terminalization() {
    ...
    store
        .finish_effect_audit(
            ...
            Some(&permit),
            identity,
            finch_runtime::EffectAuditTerminalOutcome::Acknowledged {
                response: finch_runtime::VmResumeResponse::Result { values: Vec::new() },
            },
        )
        .unwrap();
    let snapshot = store.snapshot("shared").unwrap();
    assert!(matches!(snapshot.effect_audits[0].state,
            finch_runtime::EffectAuditState::Terminal {
                outcome: finch_runtime::EffectAuditTerminalOutcome::Redacted {
                    ref outcome_kind
                }
...
fn effect_audit_observers_never_receive_secret_host_arguments() {
    ...
    for (sequence, capability, secret) in [
        (0, finch_vm::CapabilityKind::FileWrite, "FILE_SECRET_163"),
        (
            1,
        ...
        let effect = finch_vm::VmSideEffect {
            protocol_version: 1,
            sequence,
            requirement: finch_vm::CapabilityRequirement {
                capability,
            ...
            event: finch_vm::HostSideEffect::Request {
                arguments: vec![finch_vm::TypedValue::String(secret.into())],
            },
            origin: finch_vm::SourceOrigin::generated(secret),
        };
        store
    ...
    assert!(snapshot
        ...
        .any(|entry| entry.intent.identity == unbegun
            && matches!(
                entry.state,
                finch_runtime::EffectAuditState::Terminal {
                    outcome: finch_runtime::EffectAuditTerminalOutcome::AbandonedNotApplied
                }
            )));
    assert!(snapshot
        .effect_audits
        ...
        .any(|entry| entry.intent.identity == begun
            && matches!(
                entry.state,
                finch_runtime::EffectAuditState::Terminal {
                    outcome: finch_runtime::EffectAuditTerminalOutcome::UncertainProcessLoss
                }
            )));
}
...
fn effect_audit_connection_teardown_batch_is_exact_idempotent_and_retryable() {
    ...
    assert!(failed.effect_audits.iter().any(|entry| {
        entry.intent.identity == unbegun
            && matches!(entry.state, finch_runtime::EffectAuditState::IntentAccepted)
    }));
    assert!(failed.effect_audits.iter().any(|entry| {
    ...
    assert!(snapshot
        ...
        .any(|entry| entry.intent.identity == unbegun
            && matches!(entry.state,
                    finch_runtime::EffectAuditState::Terminal {
                        outcome: finch_runtime::EffectAuditTerminalOutcome::Compacted {
                            ref outcome_kind, ..
                        }
...
fn audit_bound_fixture(
    ...
    Vec<finch_runtime::EffectAuditIdentity>,
) {
...
fn test_effect_audit_batch_past_byte_bound_commits_nothing_and_holds_the_sequence() {
    ...
    store
        .set_effect_audit_journal_max_bytes_for_test(
            "shared",
            crate::effect_audit_archive::MAX_ACTIVE_JOURNAL_BYTES,
        )
        .unwrap();
    let reconciled = store
        .reconcile_effect_audits_for_disconnected_leases("shared", &[lease.lease_id])
...
fn effect_audit_epochs_bound_active_history_and_fence_replay_after_restart() {
    ...
    changed_effect.event = finch_vm::HostSideEffect::Emit {
...
fn effect_audit_stale_successor_cannot_start_but_original_permit_can_finish() {
    ...
    store
        .finish_effect_audit(
            ...
            Some(&permit),
            identity,
            finch_runtime::EffectAuditTerminalOutcome::FailedPartial {
...
fn effect_audit_schema_v14_reconstructs_as_legacy_terminal_snapshot() {
    ...
    let event = BrainEvent {
        ...
        kind: BrainEventKind::EffectRecorded {
            ...
            state: finch_vm::EffectJournalState::Acknowledged { values: Vec::new() },
        },
    };
    std::fs::write(
...
fn delivery_envelope(
    ...
    sequence: u64,
    text: &str,
) -> finch_runtime::VmEffectEnvelope {
    finch_runtime::VmEffectEnvelope {
        execution_id,
        effect: finch_vm::VmSideEffect {
            ...
            output: Vec::new(),
            event: finch_vm::HostSideEffect::Emit { text: text.into() },
            origin: finch_vm::SourceOrigin::generated("brain-delivery-test"),
        },
    }
}
...
fn output_handle_envelope(
    ...
    finch_runtime::VmEffectEnvelope {
        ...
        effect: finch_vm::VmSideEffect {
            ...
            output: Vec::new(),
            event: finch_vm::HostSideEffect::Ui {
            ...
            origin: finch_vm::SourceOrigin::generated("brain-delivery-handle"),
        },
    }
}
...
fn brain_delivery_log_replays_unacked_suffix_after_restart() {
    ...
    let client = finch_runtime::DeliveryConsumerIdentity::new(brain_id, uuid::Uuid::new_v4());
    let execution_id = uuid::Uuid::new_v4();
    ...
    assert!(store
        .acknowledge_effect_delivery(
            "shared",
            client,
            finch_runtime::DeliveryCursor::through(execution_id, 0)
        )
        .unwrap());
    assert_eq!(
        store.pending_effect_delivery("shared", client).unwrap(),
    ...
    let other = finch_runtime::DeliveryConsumerIdentity::new(brain_id, uuid::Uuid::new_v4());
    assert_eq!(
        restarted
    ...
    assert!(restarted
        .acknowledge_effect_delivery(
            "shared",
            client,
            finch_runtime::DeliveryCursor::through(execution_id, 1)
        )
        .unwrap());
    assert!(restarted
...
fn brain_delivery_log_indexes_concurrent_output_handles() {
    ...
    let download_ref = finch_runtime::OutputHandleRef::new(execution_id, "download", 4);
    assert_eq!(
        log.lock()
            .unwrap()
    ...
    let stale = finch_runtime::OutputHandleRef::new(execution_id, "download", 3);
    assert!(
        log.lock()
            .unwrap()
...
fn archive_evicts_delivery_log_so_a_reused_name_does_not_leak() {
    ...
    let client = finch_runtime::DeliveryConsumerIdentity::new(new_id, uuid::Uuid::new_v4());
    assert!(
        store
            .pending_effect_delivery("shared", client)
            .unwrap()
