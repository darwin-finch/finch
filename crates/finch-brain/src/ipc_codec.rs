...
use crate::{BrainTask, BrainTaskPriority, BrainTaskStatus};
use finch_ipc::finch_ipc_capnp::{self, brain_approval_audience};
use sha2::Digest;
...
pub fn encode_invocation_metadata(
...
pub fn decode_invocation_metadata(
...
pub fn encode_continuation_messages(
...
pub fn decode_continuation_messages(
    messages: capnp::struct_list::Reader<finch_ipc_capnp::message::Owned>,
) -> anyhow::Result<Vec<finch_providers::Message>> {
...
pub fn encode_approval_audience(
...
pub fn decode_approval_audience(
...
pub fn encode_brain_submission(
...
pub fn decode_brain_submission(
...
pub fn encode_brain_submission_outcome(
...
pub fn decode_brain_wire_reader(
            ...
            let kind = match command.which()? {
                ...
                CommandWhich::CreateSchedule(request) => {
                    ...
                    BrainRemoteCommandKind::CreateSchedule {
                        ...
                        grant_ceiling: finch_runtime::decode_effects(request.get_grant_ceiling()?)?,
...
pub fn decode_schedule(
    ...
    Ok(BrainSchedule {
        ...
        grant_ceiling: finch_runtime::decode_effects(reader.get_grant_ceiling()?)?,
        language: language_from_capnp(reader.get_language()?),
        source: text(reader.get_source()?)?,
...
pub fn encode_event(
    ...
    match &event.kind {
        BrainEventKind::MutationRecorded { outcome } => {
            ...
            match outcome {
                crate::BrainMutationOutcome::RunCancellationReserved { run_id } => {
                    recorded.set_run_cancellation_reserved(&run_id.0.to_string())
                }
                crate::BrainMutationOutcome::RunAlreadyCancelled { run_id } => {
                    recorded.set_run_already_cancelled(&run_id.0.to_string())
                }
                crate::BrainMutationOutcome::RunCancellationNoop { run_id } => {
                    recorded.set_run_cancellation_noop(&run_id.0.to_string())
                }
                crate::BrainMutationOutcome::RunCancellationDispatching {
                    run_id,
                    mutation_id,
                } => {
                ...
                crate::BrainMutationOutcome::RunCancellationReconciled {
                    run_id,
                    mutation_id,
                } => {
                ...
                crate::BrainMutationOutcome::ScheduleCancellationNoop { schedule_id } => {
                    recorded.set_schedule_cancellation_noop(&schedule_id.0.to_string())
                }
                crate::BrainMutationOutcome::HandoffCancellationNoop { handoff_id } => {
                    recorded.set_handoff_cancellation_noop(&handoff_id.0.to_string())
                }
                crate::BrainMutationOutcome::ApprovalDecisionDelivered {
                    request_seq,
                    approval_id,
                    mutation_id,
                } => {
        ...
        BrainEventKind::EffectRecorded {
            ...
            finch_runtime::encode_vm_side_effect(recorded.reborrow().init_effect(), effect)?;
            finch_runtime::encode_effect_journal_state(recorded.reborrow().init_state(), state)?;
        }
...
pub fn decode_event(
    ...
    let kind = match reader.which()? {
        Which::MutationRecorded(recorded) => {
            use crate::BrainMutationOutcome;
            ...
            let outcome = match recorded?.which()? {
                ...
                Outcome::ScheduleCancellationNoop(value) => {
                    BrainMutationOutcome::ScheduleCancellationNoop {
                        schedule_id: crate::ScheduleId(parse_uuid(value?)?),
                    }
                }
        ...
        Which::EffectRecorded(recorded) => {
            ...
            BrainEventKind::EffectRecorded {
                ...
                effect: finch_runtime::decode_vm_side_effect(recorded.get_effect()?)?,
                state: finch_runtime::decode_effect_journal_state(recorded.get_state()?)?,
            }
        }
...
mod tests {
    ...
    fn participant_submission_union_round_trips_only_client_intent() {
        let submissions = vec![
            ...
            BrainEventKind::Prompt {
                ...
                attached_mentions: vec![crate::PromptAttachment {
        ...
        let kinds = vec![
            ...
            BrainEventKind::EffectRecorded {
                ...
                effect: finch_vm::VmSideEffect {
                    ...
                    event: finch_vm::HostSideEffect::Emit {
                        text: "done".into(),
                    },
                    output: Vec::new(),
                    origin: finch_vm::SourceOrigin::generated("say"),
                },
    ...
    fn remote_brain_envelopes_round_trip_commands_and_correlated_replies() {
        ...
        accepted.mutation = Some(crate::BrainMutationReceipt {
        ...
        let schedule = BrainSchedule {
            ...
            grant_ceiling: finch_vm::EffectSet::default(),
            language: ProgramLanguage::Lisp,
    ...
    fn complete_brain_snapshot_round_trips_through_capnp() {
        ...
        let schedule_grant_ceiling = finch_vm::EffectSet(
            [finch_vm::CapabilityRequirement::file(
                finch_vm::FileOperation::Read,
                finch_vm::FileSelector::parse("reports/**").unwrap(),
            )]
            .into_iter()
            .collect(),
        );
