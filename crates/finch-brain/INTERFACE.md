# finch-brain — public interface

Generated from [`crates/finch-brain/src/lib.rs`](src/lib.rs) by `scripts/generate_interfaces.py`; CI fails if it drifts. Edit the code, then regenerate.

- **Facade:** `crates/finch-brain/src/lib.rs`
...
impl AttachedBrainClient {
    ...
    pub async fn create_schedule(&self, language: super::store::ProgramLanguage, source: String, grant_ceiling: finch_vm::EffectSet, next_due_ms: u64, interval_ms: Option<u64>, delivery_policy: super::store::BrainScheduleDeliveryPolicy) -> Result<super::store::BrainSchedule>;
...
/// Stable identity of one client projection of a Brain. Re-exported from `attachment`.
pub struct AttachmentId(pub uuid::Uuid);
/// Re-exported from `attachment`.
...
/// Exact participant/environment boundary to which a Brain-owned approval request is addressed. Re-exported from `attachment`.
...
impl BrainCredentialAuthority {
    ...
    pub fn ephemeral(signing_key: [u8; 32]) -> Self;
    ...
    pub fn invitation_tls_identity(&self) -> &finch_node::NodeTlsIdentity;
    ...
    /// Verify a signed invitation as stable descendant proof even after it has been redeemed.
    pub fn verify_invitation_descendant_proof(&self, token: &str, now_ms: u64) -> Result<BrainInvitationClaims>;
}
...
/// The one machine/workspace boundary in which a brain may cause effects. Re-exported from `projection`.
pub struct BrainEnvironment { … }
/// Re-exported from `journal`.
...
/// Stable identity of one durable Brain. Re-exported from `journal`.
pub struct BrainId(pub uuid::Uuid);
/// Reviewed, immutable program that establishes a Brain's initial typed state. Re-exported from `schedule`.
...
/// Durable identity and preconditions for one authorized Brain mutation. Re-exported from `journal`.
pub struct BrainMutationReceipt { … }
/// Re-exported from `journal`.
pub struct BrainProgram { … }
/// Secret-free provider/model overlay stored on a named Brain. Re-exported from `journal`.
...
pub enum BrainRemoteEnvelope { Projection, Command, Reply }
...
/// Re-exported from `schedule`.
pub struct BrainSchedule { … }
/// Re-exported from `schedule`.
pub enum BrainScheduleDeliveryPolicy { Coalesce, BoundedCatchUp }
/// One durable schedule delivery and the queued run that owns it. Re-exported from `schedule`.
pub struct BrainScheduleDue { … }
/// Durable, non-authority-bearing identity for a reviewed module scheduled by the Brain itself. Re-exported from `schedule`.
pub struct BrainScheduleModuleIdentity { … }
/// Re-exported from `projection`.
...
impl BrainStore {
    pub async fn acquire_run_publication(&self, name: &str, run_id: RunId) -> Result<tokio::sync::OwnedMutexGuard<RunPublicationGate>>;
    pub async fn clear_run_cancellation(&self, name: &str, run_id: RunId) -> Result<()>;
    ...
    pub async fn reserve_run_publication_cancellation(&self, name: &str, run_id: RunId) -> Result<()>;
    /// A normally returned runner can prove that accepted-but-unbegun reservations were never dispatched.
    pub fn abandon_unbegun_effect_audits(&self, grant: &EffectAuditAuthorityGrant) -> Result<usize>;
    ...
    pub fn acknowledge_effect_delivery(&self, name: &str, consumer: finch_runtime::DeliveryConsumerIdentity, cursor: finch_runtime::DeliveryCursor) -> Result<bool>;
    ...
    pub fn begin_effect_audit(&self, grant: &EffectAuditAuthorityGrant, identity: finch_runtime::EffectAuditIdentity) -> Result<finch_runtime::HostEffectPermit>;
    pub fn begin_run_approval_for_connection(&self, name: &str, attachment_id: AttachmentId, connection_id: ConnectionId, request_seq: u64, approval_id: String, approval_kind: String, subject: String, audience: BrainApprovalAudience, detail: serde_json::Value) -> Result<RunId>;
    pub fn bind_run_connection(&self, name: &str, run_id: RunId, attachment_id: AttachmentId, connection_id: ConnectionId) -> Result<()>;
    ...
    pub fn commit_runner_runtime(&self, name: &str, request_seq: u64, runtime_revision: u64, checkpoint: finch_vm::TypedRuntimeCheckpoint) -> Result<BrainEvent>;
    pub fn commit_runner_runtime_for_run(&self, name: &str, run_id: RunId, request_seq: u64, runtime_revision: u64, checkpoint: finch_vm::TypedRuntimeCheckpoint) -> Result<BrainEvent>;
    /// Journal the latest checkpoint only after a ProgramRuntime commit.
    pub fn commit_runtime(&self, name: &str, request_seq: u64, runtime_revision: u64, runtime: &finch_runtime::ProgramRuntime) -> Result<BrainEvent>;
    ...
    pub fn complete_reserved_run_cancellation_on_disconnect(&self, name: &str, sender: &str, run_id: RunId) -> Result<bool>;
    /// How many Brains this store would answer for, without hydrating any.
    pub fn count_unhydrated(&self) -> usize;
    pub fn create_schedule(&self, name: &str, created_by: &str, initiating_attachment_id: AttachmentId, language: ProgramLanguage, source: impl Into<String>, grant_ceiling: finch_vm::EffectSet, next_due_ms: u64, interval_ms: Option<u64>, delivery_policy: BrainScheduleDeliveryPolicy) -> Result<BrainSchedule>;
    pub fn create_schedule_with_receipt(&self, name: &str, created_by: &str, initiating_attachment_id: AttachmentId, language: ProgramLanguage, source: String, grant_ceiling: finch_vm::EffectSet, next_due_ms: u64, interval_ms: Option<u64>, delivery_policy: BrainScheduleDeliveryPolicy, mutation: Option<BrainMutationReceipt>) -> Result<BrainSchedule>;
    ...
    pub fn effect_delivery_log(&self, name: &str) -> Result<Option<Arc<std::sync::Mutex<finch_runtime::VmEffectDeliveryLog>>>>;
    pub fn environment(&self) -> &BrainEnvironment;
    /// Drop the in-memory copy of a Brain without touching its durable state or its index entries.
    pub fn evict_resident_brain_for_tests(&self, name: &str) -> bool;
    ...
    pub fn fail_next_effect_audit_batch_for_test(&self, name: &str) -> Result<()>;
    /// Record a monotonic outcome.
    pub fn finish_effect_audit(&self, grant: &EffectAuditAuthorityGrant, permit: Option<&finch_runtime::HostEffectPermit>, identity: finch_runtime::EffectAuditIdentity, outcome: finch_runtime::EffectAuditTerminalOutcome) -> Result<()>;
    ...
    pub fn issue_effect_audit_authority(&self, name: &str, run_id: RunId, lease_id: RunnerLeaseId, _connection_id: Option<ConnectionId>) -> Result<EffectAuditAuthorityGrant>;
    ...
    pub fn pause_after_cancellation_reservation_for_test(&self) -> (std::sync::mpsc::Receiver<()>, std::sync::mpsc::Sender<()>);
    ...
    pub fn pending_effect_delivery(&self, name: &str, consumer: finch_runtime::DeliveryConsumerIdentity) -> Result<Vec<finch_runtime::VmEffectEnvelope>>;
    /// Packed Runtime/Application ABI frames for the unacknowledged suffix.
    pub fn pending_effect_delivery_frames(&self, name: &str, consumer: finch_runtime::DeliveryConsumerIdentity) -> Result<Vec<Vec<u8>>>;
    pub fn pending_reserved_cancellations_for_attachment(&self, name: &str, attachment_id: AttachmentId) -> Result<Vec<RunId>>;
    ...
    pub fn program_runtime(&self, name: &str) -> Result<Arc<finch_runtime::ProgramRuntime>>;
    ...
    pub fn prune_run_publication(&self, name: &str, run_id: RunId) -> Result<()>;
    ...
    /// Durably reconcile every unresolved audit whose exact runner lease was owned by a transport connection that has terminated.
    pub fn reconcile_effect_audits_for_disconnected_leases(&self, name: &str, lease_ids: &[RunnerLeaseId]) -> Result<usize>;
    /// Persist envelopes before local Brain handling.
    pub fn record_effect_delivery(&self, name: &str, envelopes: &[finch_runtime::VmEffectEnvelope]) -> Result<()>;
    ...
    pub fn reserve_effect_audit(&self, grant: &EffectAuditAuthorityGrant, execution_id: uuid::Uuid, effect: finch_vm::VmSideEffect) -> Result<finch_runtime::EffectAuditIdentity>;
    /// How many Brains are actually resident in memory, i.e.
    pub fn resident_brain_count(&self) -> usize;
    pub fn retire_connection_and_owned_active_runs(&self, name: &str, attachment_id: AttachmentId, connection_id: ConnectionId) -> Result<Vec<RunId>>;
    /// Resolve an exact durable reserve retry before applying current callback, lease, or run-terminal checks.
    pub fn retry_effect_audit_reservation(&self, grant: &EffectAuditAuthorityGrant, execution_id: uuid::Uuid, effect: &finch_vm::VmSideEffect) -> Result<Option<finch_runtime::EffectAuditIdentity>>;
    /// On-disk directory that holds named Brain folders, if this store persists.
    pub fn root(&self) -> Option<&std::path::Path>;
    /// Durable cancellation intent is itself the publication fence.
    pub fn run_cancellation_reserved(&self, name: &str, run_id: RunId) -> Result<bool>;
    /// Return the durable reducible state a newly connected environment runner must install before accepting ProgramRuns.
    pub fn runner_checkpoint(&self, name: &str) -> Result<(u64, finch_vm::TypedRuntimeCheckpoint)>;
    pub fn schedule_disconnect_terminalization_retry(&self, name: String, sender: String, run_id: RunId, request_seq: u64, status: BrainRunStatus, detail: String);
    ...
    pub fn schedule_reserved_cancellation_retry(&self, name: String, sender: String, run_id: RunId);
    ...
    pub fn seed_history_for_test(&self, name: &str, events: u64) -> Result<()>;
    pub fn seed_mature_effect_audit_history_for_test(&self, name: &str, records: usize, epochs: usize) -> Result<()>;
    ...
    pub fn terminalize_run_with_result_if_active(&self, name: &str, sender: &str, run_id: RunId, request_seq: u64, status: BrainRunStatus, detail: String) -> Result<Option<BrainEvent>>;
    ...
    pub fn with_test_environment_generation(machine: impl Into<String>, root: Option<PathBuf>, generation: u64) -> Self;
}
...
/// One MemTree leaf the query processor has promoted into this Brain's durable, byte-stable recall prefix (#940). Re-exported from `journal`.
pub struct CommittedMemoryRecord { … }
/// Identity of one live transport connection for a durable attachment. Re-exported from `attachment`.
pub struct ConnectionId(pub uuid::Uuid);
/// Opaque daemon-side authority for one runner capability.
pub struct EffectAuditAuthorityGrant { … }
...
pub struct IsolatedTestSocketIdentity { … }
/// Re-exported from `schedule`.
pub enum ProgramLanguage { Forth, Lisp }
/// Path + digest + payload for one `@` mention attached to a Prompt. Re-exported from `journal`.
...
impl RemoteBrainClient {
    ...
    pub async fn create_schedule(&self, language: super::store::ProgramLanguage, source: String, grant_ceiling: finch_vm::EffectSet, next_due_ms: u64, interval_ms: Option<u64>, delivery_policy: super::store::BrainScheduleDeliveryPolicy) -> Result<super::store::BrainSchedule>;
    pub async fn create_schedule_with_handle(&self, language: super::store::ProgramLanguage, source: String, grant_ceiling: finch_vm::EffectSet, next_due_ms: u64, interval_ms: Option<u64>, delivery_policy: super::store::BrainScheduleDeliveryPolicy, handle: &BrainMutationHandle) -> Result<super::store::BrainSchedule>;
    ...
    pub async fn prepare_create_schedule_mutation(&self, language: super::store::ProgramLanguage, source: &str, grant_ceiling: &finch_vm::EffectSet, next_due_ms: u64, interval_ms: Option<u64>, delivery_policy: super::store::BrainScheduleDeliveryPolicy) -> Result<BrainMutationHandle>;
    ...
    pub async fn test_send_remote_command(&self, kind: crate::test_support::BrainRemoteCommandKind) -> Result<crate::test_support::BrainRemoteReply>;
    pub async fn test_send_remote_command_with_handle(&self, kind: crate::test_support::BrainRemoteCommandKind, handle: Option<&BrainMutationHandle>) -> Result<crate::test_support::BrainRemoteReply>;
...
pub trait LocalBrainTransport {
    ...
    async fn brain_create_schedule(&self, brain: &str, attachment: &BrainAttachment, language: super::store::ProgramLanguage, source: &str, grant_ceiling: &finch_vm::EffectSet, next_due_ms: u64, interval_ms: Option<u64>, delivery_policy: &super::store::BrainScheduleDeliveryPolicy) -> Result<super::store::BrainSchedule>;
...
```rust
#[cfg(all(any(test, feature = "test-support"), unix))]
pub fn authenticate_isolated_test_peer(stream: &tokio::net::UnixStream) -> anyhow::Result<()> { … }
...
pub fn decode_approval_audience(reader: brain_approval_audience::Reader<'_>) -> anyhow::Result<BrainApprovalAudience> { … }
pub fn decode_attachment(reader: finch_ipc_capnp::brain_attachment::Reader<'_>) -> anyhow::Result<BrainAttachment> { … }
pub fn decode_brain_remote_envelope(bytes: &[u8]) -> anyhow::Result<BrainRemoteEnvelope> { … }
pub fn decode_brain_submission(reader: finch_ipc_capnp::brain_submission::Reader<'_>) -> anyhow::Result<BrainEventKind> { … }
pub fn decode_brain_wire_reader(root: finch_ipc_capnp::brain_wire_message::Reader<'_>) -> anyhow::Result<BrainWireMessage> { … }
pub fn decode_continuation_messages(messages: capnp::struct_list::Reader<finch_ipc_capnp::message::Owned>) -> anyhow::Result<Vec<finch_providers::Message>> { … }
pub fn decode_environment(reader: finch_ipc_capnp::brain_environment::Reader<'_>) -> anyhow::Result<BrainEnvironment> { … }
pub fn decode_event(reader: finch_ipc_capnp::brain_event::Reader<'_>) -> anyhow::Result<BrainEvent> { … }
pub fn decode_invocation_metadata(reader: finch_ipc_capnp::invocation_metadata::Reader<'_>) -> anyhow::Result<finch_providers::InvocationMetadata> { … }
pub fn decode_messages(messages: capnp::struct_list::Reader<finch_ipc_capnp::message::Owned>) -> anyhow::Result<Vec<finch_providers::Message>> { … }
pub fn decode_run(reader: finch_ipc_capnp::brain_run::Reader<'_>) -> anyhow::Result<BrainRun> { … }
pub fn decode_runner_handoff(reader: finch_ipc_capnp::brain_runner_handoff::Reader<'_>) -> anyhow::Result<BrainRunnerHandoff> { … }
pub fn decode_runner_lease(reader: finch_ipc_capnp::brain_runner_lease::Reader<'_>) -> anyhow::Result<BrainRunnerLease> { … }
pub fn decode_schedule(reader: finch_ipc_capnp::brain_schedule::Reader<'_>) -> anyhow::Result<BrainSchedule> { … }
pub fn decode_snapshot(reader: finch_ipc_capnp::brain_snapshot::Reader<'_>) -> anyhow::Result<BrainSnapshot> { … }
...
pub fn directory_listing_for_tests(path: &std::path::Path) -> String { … }
pub fn encode_approval_audience(mut builder: brain_approval_audience::Builder<'_>, audience: &BrainApprovalAudience) { … }
pub fn encode_attachment(mut builder: finch_ipc_capnp::brain_attachment::Builder<'_>, attachment: &BrainAttachment) { … }
pub fn encode_brain_remote_envelope(envelope: &BrainRemoteEnvelope) -> anyhow::Result<Vec<u8>> { … }
pub fn encode_brain_submission(mut builder: finch_ipc_capnp::brain_submission::Builder<'_>, kind: &BrainEventKind) -> anyhow::Result<()> { … }
pub fn encode_brain_submission_outcome(mut builder: finch_ipc_capnp::brain_submission_outcome::Builder<'_>, accepted: &BrainEvent, run: Option<&BrainRun>, result: Option<&BrainEvent>) -> anyhow::Result<()> { … }
pub fn encode_continuation_messages(builder: capnp::struct_list::Builder<finch_ipc_capnp::message::Owned>, messages: &[finch_providers::Message]) -> anyhow::Result<()> { … }
pub fn encode_environment(mut builder: finch_ipc_capnp::brain_environment::Builder<'_>, environment: &BrainEnvironment) { … }
pub fn encode_event(mut builder: finch_ipc_capnp::brain_event::Builder<'_>, event: &BrainEvent) -> anyhow::Result<()> { … }
pub fn encode_invocation_metadata(mut builder: finch_ipc_capnp::invocation_metadata::Builder<'_>, metadata: &finch_providers::InvocationMetadata) { … }
pub fn encode_messages(mut builder: capnp::struct_list::Builder<finch_ipc_capnp::message::Owned>, messages: &[finch_providers::Message]) -> anyhow::Result<()> { … }
pub fn encode_run(mut builder: finch_ipc_capnp::brain_run::Builder<'_>, run: &BrainRun) { … }
pub fn encode_runner_handoff(mut builder: finch_ipc_capnp::brain_runner_handoff::Builder<'_>, handoff: &BrainRunnerHandoff) { … }
pub fn encode_runner_lease(mut builder: finch_ipc_capnp::brain_runner_lease::Builder<'_>, lease: &BrainRunnerLease) { … }
pub fn encode_schedule(mut builder: finch_ipc_capnp::brain_schedule::Builder<'_>, schedule: &BrainSchedule) { … }
pub fn encode_snapshot(mut builder: finch_ipc_capnp::brain_snapshot::Builder<'_>, snapshot: &BrainSnapshot) -> anyhow::Result<()> { … }
...
pub fn run_status_from_capnp(status: finch_ipc_capnp::BrainRunStatus) -> BrainRunStatus { … }
pub fn run_status_to_capnp(status: BrainRunStatus) -> finch_ipc_capnp::BrainRunStatus { … }
...
#[cfg(any(test, feature = "test-support"))]
pub fn seed_scheduled_brain_for_tests(store: &BrainStore, name: &str, next_due_ms: u64) -> (AttachmentId, ScheduleId) { … }
#[cfg(all(any(test, feature = "test-support"), unix))]
pub fn supervised_test_subprocess_command() -> std::process::Command { … }
...
#[cfg(all(any(test, feature = "test-support"), unix))]
pub fn validate_isolated_test_socket(proof: &IsolatedTestProof, path: &std::path::Path) -> anyhow::Result<IsolatedTestSocketIdentity> { … }
```
