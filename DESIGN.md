...
| **`transport`** (3): domain-neutral Cap'n Proto schema/protocol/socket core, node identity, service discovery | `crates/finch-ipc`, `node`, `network`, `service`, `node_name.rs` | IPC capsule [`crates/finch-ipc/AGENTS.md`](crates/finch-ipc/AGENTS.md), interface [`crates/finch-ipc/INTERFACE.md`](crates/finch-ipc/INTERFACE.md); application client adapter in `src/client`, daemon RPC adapter in `src/server`, Brain codec in `src/brain/ipc_codec.rs`, runtime checkpoint codec in `crates/finch-runtime/src/ipc_codec.rs`; Node capsule [`src/node/AGENTS.md`](src/node/AGENTS.md), interface [`src/node/INTERFACE.md`](src/node/INTERFACE.md); wire schema in [`crates/finch-ipc/schema/finch_ipc.capnp`](crates/finch-ipc/schema/finch_ipc.capnp) |
