...
| **`brain`** (4): durable named Brains, the HTTP server and runner, daemon lifecycle | `src/brain`, `server`, `daemon`, `client`, `agent`, `review`, `registry` (migration only), `graph` | Brain capsule [`src/brain/AGENTS.md`](src/brain/AGENTS.md), interface [`src/brain/INTERFACE.md`](src/brain/INTERFACE.md); Daemon capsule [`src/daemon/AGENTS.md`](src/daemon/AGENTS.md), interface [`src/daemon/INTERFACE.md`](src/daemon/INTERFACE.md); see the [Brain test inventory](tests/BRAIN_TEST_INVENTORY.md) |
