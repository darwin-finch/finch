...
| **`routing-tree`** (0): the dependency-free binary routing tree over embeddings — fitted PCA axes, dual-insert, stability-gated splitting, adaptive/beam search, verified removal — and its durable-row save/load codec; owns no schema and no hydration lifecycle | `crates/finch-routing-tree` | [README](crates/finch-routing-tree/README.md), [agent contract](crates/finch-routing-tree/AGENTS.md), [facade](crates/finch-routing-tree/src/lib.rs) |
| **`memory`** (1): SQLite-backed store, hydration, retrieval coverage, and opaque program index rows over the routing-tree crate | `crates/finch-memory`; root [`src/program_registry.rs`](src/program_registry.rs) maps program identity to memory rows | [Memory README](crates/finch-memory/README.md), [agent contract](crates/finch-memory/AGENTS.md), [facade](crates/finch-memory/src/lib.rs) |
| **`source-index`** (0): exact-byte source identity, bounded spans and provenance, body-free structural outlines, Git-defined directory menus, quota-bounded atomic repository cache, and stale-generation validation; tools own sibling-hop ranking over this facade | `src/source_index` | [README](src/source_index/README.md), [agent contract](src/source_index/AGENTS.md), [facade](src/source_index/mod.rs) |
